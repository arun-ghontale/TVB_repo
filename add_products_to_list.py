...
flipkart_data = pd.read_csv('flipkart_com-ecommerce_sample.csv',encoding = 'utf-8')

product_names = flipkart_data['product_name'] 
product_descriptions = flipkart_data['description']
product_prices = flipkart_data['retail_price']

for product_name,product_description,product_price in zip(product_names,product_descriptions,product_prices):
	try:
		payload = {'optype':'add','name':clean(product_name),'price':product_price,'description':clean(product_description)}
		print(payload)
		session = requests.Session()
		session.post('http://127.0.0.1:5500/api/products',headers=headers,data=payload)
	except Exception as e: 
